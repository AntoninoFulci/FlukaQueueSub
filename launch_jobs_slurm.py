...
echo
echo Job completed. Transferring file to $pog_dir

mv ./*.root $pog_dir

echo
echo Catting err file here
...
def generate_sh(input, iteration, fluka_path, custom_exe, mem, ntasks, nodes, time):
    pog_dir = os.getcwd()
...
def launch_jobs(input_file, job_number, custom_exe, queue, mem, ntasks, nodes, time, dry_run, output_dir):
    ...
    for i in range(1, job_number + 1):
        ...
        generate_sh(new_input, i, FLUKA_PATH, custom_exe, mem, ntasks, nodes, time)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Launch FLUKA jobs on the farm',
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="Example usage:\n ./launch_jobs.py -f input.inp -n 10 -c custom_exe -q queue -m 1500 -t 1 -o 1 -T 1-00:00:00")
    ...
    parser.add_argument('-o', '--nodes', type=int, default=1, help='Number of nodes for the job')
    ...
    table = [
        ...
        ["-o",      f"{C}Number of nodes{RE}",      f"{C}{args.nodes}{RE}"],
    ...
    launch_jobs(args.input, args.njobs, args.custom_exe, args.queue, args.mem, args.ntasks, args.nodes, args.time, args.dry_run, args.output_dir)